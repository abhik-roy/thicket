...
import { DatasetCodeManager } from '../components/DatasetCodeManager'
...
export function DatasetScreen({ coderId, passNo, codebookId, theme, onToggleTheme, onOpenWorkspace }: Props) {
  ...
  return <main className="app-shell min-h-screen">
    <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/95 px-4 py-3 shadow-sm backdrop-blur sm:px-6"><div className="mx-auto flex max-w-[92rem] flex-wrap items-center gap-3"><Link to="/" onClick={(event) => { if (!canLeaveEditor()) event.preventDefault() }} className="btn-secondary inline-flex items-center">← Work queue</Link><div><p className="eyebrow">Inductive analysis</p><h1 className="text-lg font-bold text-slate-900">Evidence dataset</h1></div><div className="ml-auto hidden text-xs text-slate-500 sm:block">{coderId} · Pass {passNo}</div><details className="relative"><summary className="btn-secondary flex cursor-pointer list-none items-center">Export ▾</summary><div className="surface absolute right-0 z-30 mt-2 grid min-w-48 gap-1 rounded-xl p-2 shadow-lg"><a className="rounded-lg px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50" href={exportUrl} download>CSV data</a><a className="rounded-lg px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50" href={pdfExportUrl} download>PDF card report</a></div></details><HeaderActions theme={theme} onToggleTheme={onToggleTheme} onOpenWorkspace={onOpenWorkspace} /></div></header>
    <div className="mx-auto grid max-w-[92rem] items-start gap-5 px-4 py-5 md:grid-cols-[15rem_minmax(0,1fr)] sm:px-6">
      ...
      <div className={`space-y-3 md:sticky md:top-20 ${navigationOpen ? 'block' : 'hidden md:block'}`}><aside className="surface rounded-xl p-3">
      ...
      </aside><DatasetCodeManager codes={codes} segments={segments} codebookId={codebookId} /></div>
